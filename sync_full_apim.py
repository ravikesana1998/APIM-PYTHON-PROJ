...
API_VERSION = os.getenv("API_VERSION")  # 'v1', 'v2', etc.
API_VERSION_SET_ID = f"{os.getenv('AZURE_APIM_API_ID')}-versionset"
AZURE_APIM_API_ID = f"{os.getenv('AZURE_APIM_API_ID')}-{API_VERSION}"
...
def ensure_api_exists():
    result = subprocess.run(
        f"az apim api show --resource-group {AZURE_RESOURCE_GROUP} "
        f"--service-name {AZURE_APIM_NAME} --api-id {AZURE_APIM_API_ID}",
        shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    if result.returncode != 0:
        ...
        run(
            ...
            f"--path {AZURE_APIM_API_ID} --display-name {AZURE_APIM_API_ID} "
            f"--specification-format OpenApi --specification-path {SWAGGER_FILE} "
            f"--api-version {API_VERSION} --api-version-set-id {API_VERSION_SET_ID} "
            f"--versioning-scheme Segment"
        )
    else:
        print("✅ API already exists in APIM.")
...
def sync_operations():
    for fname in os.listdir(SPLIT_DIR):
        ...
        # Extract path params (for create)
        template_params = re.findall(r"{(.*?)}", swagger_path)
        template_args = ""
        for param in template_params:
            template_args += (
